...
% -------------------- embedded bibliography (all 19 verified against DBLP/publisher/DOI) --------------------
...
all twelve configurations tested. A 12-hour-ahead ensemble forecast captures 99--100\% of the benefit
available to perfect foresight at an identical optimisation budget. We also report the boundary of the
...
\item Implement the scheduler and an ensemble carbon-intensity forecaster on real data (Google Cluster Trace and UK National Grid carbon intensity).
...
no reproducible artefact for job-level scheduling research. None of these works casts carbon-aware scheduling
as a metaheuristic search problem.
...
outperform plain recurrent networks. What remains comparatively under-explored is the tight coupling of a
strong forecaster into the scheduling loop: many carbon-aware schedulers still act on the \emph{current}
intensity, leaving the accuracy gains of forecasting only partially realised.
...
optimise energy, cost or makespan, have not been applied to carbon, and frequently trade SLA compliance for
objective gains \cite{kalra2015review, mirjalili2016whale, heidari2019harris}; and although carbon intensity
is demonstrably forecastable, forecasts are often used reactively \cite{leerbeck2020short, maji2022carboncast}.
...
peak does not coincide with the carbon peak, cost is effectively constant across methods---0.755~GBP for every
method in the published configuration---and therefore does not influence any ranking. It is retained for
...
experiments use fixed seeds and pinned dependencies (Mealpy 3.0.3, NumPy 1.26.0, SciPy 1.16.3,
TensorFlow with operation-level determinism enabled, and scikit-learn).
...
\caption{CA-WOA versus the FIFO baseline and standard metaheuristics under the temporal model: carbon reduction (left) and SLA violations (right).}\label{fig:temporal}
\end{figure}
...
\caption{Host-capacity model across all methods: carbon reduction, SLA violations and server utilisation (CA-WOA highlighted).}\label{fig:capacityfig}
\end{figure}
...
Runtime scales linearly in task count. WOA is the cheapest search in the study (1.57~s and 1.0~MB at 500
tasks, rising to 6.23~s and 4.9~MB at 3000), and GA the most expensive (2.65 to 10.68~s). At the counts used
in comparable studies, runtimes are 0.89 to 1.25~s for WOA over 50 to 300 tasks.
...
\caption{Held-out carbon-intensity forecast: the actual series compared with the baseline LSTM and the adopted ensemble.}\label{fig:forecast}
\end{figure}
...
12-hour-ahead ensemble forecast captures 99--100\% of the benefit available to perfect foresight.
...
[Zenodo DOI to be inserted on acceptance]. Dependencies are pinned (Mealpy 3.0.3, NumPy 1.26.0, SciPy 1.16.3)
and TensorFlow operation-level determinism is enabled, so all reported figures reproduce exactly.
