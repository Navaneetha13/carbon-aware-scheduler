...
The real datasets are ingested directly: the Google Cluster Trace task-event files and the NASA-iPSC log are
parsed to extract task submission times, durations and CPU requests, and the five regional carbon-intensity
series listed in Table~\ref{tab:regions} are loaded as the per-slot carbon signal. The principal outputs are
the trained forecasting models, the per-method scheduling results (feasibility, carbon, SLA, energy,
utilisation, makespan and overload), the 30-seed scalability results across 500--3000 tasks on 10 and 20
hosts, the runtime and memory figures at 50--300 tasks, and the figures presented in
...
The first experiment compares CA-WOA against the FIFO baseline and the five standard metaheuristics at the
largest workload, 3000 tasks on 10 hosts, over 30 seeds. Fig.~\ref{fig:temporal} reports carbon reduction and
deadline violations, and Table~\ref{tab:alg} gives the figures.

CA-WOA achieves the highest carbon reduction, $87.58 \pm 0.10$\%, while holding deadline violations to
0.13\%. Standard WOA reaches 87.15\% with 0.57\% violations, so carbon-aware seeding contributes $+0.44$
...
The important contrast is on the second axis. PSO, DE and GA reach \emph{lower} carbon reduction than CA-WOA
\emph{and} miss 61.5\%, 54.0\% and 44.3\% of deadlines respectively, so they are worse on both measures at
...
\subsection{Multiple workload subsets and a second trace}\label{sec:workloads}
Reviewers asked whether the results depend on the particular workload sample drawn.
Two checks were run. First, four \emph{disjoint} subsets of the Google trace were
scheduled independently. CA-WOA's carbon reduction varies little across them:
...
Second, the NASA-iPSC log was used as an independent workload with a genuine 92-day
...
The ranking, however, is unchanged: CA-WOA is best in all six NASA configurations
...
realistic arrival process, which is the pattern the submitted version claimed but
...
of roughly 12\% is therefore withdrawn. Fig.~\ref{fig:forecast} reports the accuracy of every model against
horizon, together with the three naive baselines.

Coupling the forecast into the scheduler makes it predictive rather than reactive, and the effect is measured
at an identical optimisation budget of 4{,}840 objective evaluations. At 1000 tasks on 10 hosts, scheduling
against a 12-hour-ahead forecast yields 1.070~kg~CO\textsubscript{2} against 1.224~kg for purely reactive
scheduling on the current intensity, while a perfect-foresight oracle reaches 1.024~kg. The forecast therefore
recovers 76.8\% of the advantage that perfect knowledge of the future would confer; across the three
configurations tested the range is 73--89\%.

\textbf{An earlier version of this analysis reported 78\%, computed from a different and smaller
...
\paragraph{Sensitivity to forecast error.} Because a forecast-driven scheduler is only as good as its
...
The degradation is gradual rather than sharp. A realised MAE of 9.9~gCO\textsubscript{2}/kWh---already
...
For completeness, a Transformer forecaster was also tested but performed markedly worse (MAE
9.27~gCO\textsubscript{2}/kWh against 3.51 for gradient boosting on the same single-run protocol), consistent
with reports that attention models underperform on smooth, short time series; this negative result is
...
overload-penalised fitness function; the scheduler and an ensemble carbon forecaster were implemented on the
real Google Cluster and NASA-iPSC traces and five regional grid carbon-intensity series; and the approach was
evaluated against deterministic baselines, five standard metaheuristics and two published schedulers across
task counts from 500 to 3000 on 10 and 20 hosts, over 30 independent seeds.
...
\url{https://doi.org/10.5281/zenodo.22125797}. Dependencies are recorded (Mealpy 3.0.3, NumPy 2.5.2, SciPy 1.16.3)
