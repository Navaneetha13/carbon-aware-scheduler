...
forecast error.
\end{revcomment}
\rsp{Controlled Gaussian error is injected into the carbon signal, the schedule is chosen on the
\emph{noisy} signal, and the result is scored against the \emph{true} signal, over ten seeds at each of six
error levels. The degradation is gradual rather than sharp: a realised MAE of 9.9~gCO\textsubscript{2}/kWh
costs only 0.33 percentage points of carbon reduction, and even a severe 71.6~gCO\textsubscript{2}/kWh
error---roughly two thirds of the mean United Kingdom intensity---costs 1.55 points. The method therefore does
not require an accurate forecast so much as one that ranks slots roughly correctly, which is a weaker
requirement. At the smallest error levels the noisy schedule occasionally scores marginally above the
perfect-information schedule; this is a sampling artefact of greedy tie-breaking rather than a real gain, and
