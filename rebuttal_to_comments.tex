...
to the best competing method. Both are in Section~6 (Limitations).
...
\end{comment}
\R{The endpoints are now justified in the text as a mid-range two-socket volume
...
\Where{Section~3.2 (energy and carbon model); Sections~5.6, 5.7 and 5.8;
Tables and figures therein.}

\subsection*{Novelty claim}
\begin{comment}{Comment R2.2}
...
\end{comment}
\R{The priority claim has been removed from the abstract, the introduction and
...
\Where{Abstract; Section~1 (contributions); Section~2.3; Section~2.5;
Table~1; Section~5.2.}

\subsection*{Ablation study}
\begin{comment}{Comment R2.3}
...
\end{comment}
\R{A five-arm ablation is reported under both energy models, with 30 seeds. Two
...
\Where{Section~5.4 and Table~6; Section~5.11 (forecast coupling).}

\subsection*{Fitness-weight sensitivity}
\begin{comment}{Comment R2.4}
Evaluate different $\alpha$, $\beta$ and $\gamma$ values and justify the selected
weights $\alpha = 0.4$, $\beta = 0.3$ and $\gamma = 0.3$.
\end{comment}
\R{Eleven weight combinations were evaluated over 30 seeds. Read on carbon alone
...
optimal choice.}
\Where{Section~5.5; Table~6 (rows with $\beta=0$ and $\gamma=0$).}

\subsection*{SLA validation}
\begin{comment}{Comment R2.5}
...
claim.
\end{comment}
\R{Absolute violation counts are now reported alongside percentages, and raw
...
axes simultaneously.}
\Where{Section~3.3 (metrics); Section~5.3; Table~5.}

\subsection*{Statistical validation}
\begin{comment}{Comment R2.6}
...
\end{comment}
\R{Every algorithm is now run over 30 independent seeds. Each comparison reports
...
\Where{Section~3.3; all result tables; archived per-seed CSV files.}

\subsection*{Forecasting validation}
\begin{comment}{Comment R2.7}
...
\end{comment}
\R{The forecaster is now evaluated with direct multi-step prediction at horizons of
...
\Where{Section~3.3; Section~5.10; Table~9; Figure~6.}

\subsection*{Forecast--scheduling coupling}
\begin{comment}{Comment R2.8}
...
\end{comment}
\R{The coupling is now described explicitly and measured. A 12-hour-ahead
...
\Where{Section~5.9; Figure~5; runtime and memory tables.}

\subsection*{Reproducibility}
\begin{comment}{Comment R2.10}
...
\end{comment}
\R{All code, data, per-seed raw results and the analysis scripts are archived at
...
\Where{Section~3.3; Availability of Data and Materials; archived artefact.}
...
\begin{comment}{Comment R2.11}
Ablation: WOA; WOA + carbon-aware seeding; CA-WOA without SLA term; CA-WOA without
overload/cost term; full CA-WOA.
\end{comment}
\R{All five arms are reported, plus the greedy-only baseline and the carbon-blind
...
submission ordering.}
\Where{Section~5.4; Section~3.1.}

\begin{comment}{Comment R2.12}
Compare against an earliest-deadline-first + lowest-carbon-slot heuristic and a
constrained-scheduling optimization method.
\end{comment}
\R{The EDF + lowest-carbon-slot heuristic is implemented and appears in every
...
\Where{Section~4.4 (formulation and Lemma~1); Section~5.3; all results tables.}

\begin{comment}{Comment R2.13}
Sensitivity analysis for fitness weights and seeded-population percentage.
\end{comment}
\R{Both are reported: eleven weight combinations and six seeded fractions from 0 to
1, each over 30 seeds. See Comment~R2.4.}
\Where{Section~5.5.}

\begin{comment}{Comment R2.14}
Use at least 20--30 independent random seeds, or provide a statistically justified
alternative with confidence intervals.
\end{comment}
\R{Thirty independent seeds are used throughout, with 95\,\% confidence intervals,
two significance tests and two effect-size measures. See Comment~R2.6.}
\Where{Section~3.3; all results tables.}

\begin{comment}{Comment R2.15}
Report convergence curves and total objective-function evaluations.
\end{comment}
\R{Convergence curves are reported per epoch with dispersion bands, and the
evaluation count is recorded for every run---which is how the unequal-budget
problem was detected. See Comment~R2.9.}
\Where{Figure~5; Section~5.9.}

\begin{comment}{Comment R2.16}
Report runtime and memory consumption for all algorithms at 50, 100, 200 and 300
tasks.
\end{comment}
\R{Runtime and memory are reported at exactly those four task counts, in addition
to the 500--3000 range used for the main study. WOA runs in 0.89\,s at 50 tasks and
1.24\,s at 300.}
\Where{Section~5.9.}

\begin{comment}{Comment R2.17}
Evaluate multiple workload subsets rather than relying on one fixed 60-task sample.
\end{comment}
\R{Four disjoint subsets of the Google trace are evaluated, and the NASA-iPSC log
from the Parallel Workloads Archive is added as an independent workload with a
genuine 92-day arrival process. CA-WOA is stable across the Google subsets
(standard deviation 0.13 to 0.59) and wins all six NASA configurations. On NASA the
SLA contrast is far starker: PSO, DE and GA miss 38--64\,\% of deadlines while
CA-WOA stays below 0.5\,\%.}
\Where{Section~3.1; Section~5.12.}

\begin{comment}{Comment R2.18}
Test heterogeneous host capacities and more realistic power-consumption models.
\end{comment}
\R{Both are now tested: three fleet compositions including an adversarial one, and
three power models. See Comment~R2.1.}
\Where{Sections~5.6 and 5.8.}

\begin{comment}{Comment R2.19}
Evaluate multiple carbon-intensity traces/regions or justify the representativeness
of the UK signal.
\end{comment}
\R{Five real half-hourly regional traces are now used, spanning the same 111-day
...
\Where{Section~3.1 and Table~2; Section~5.12; Figure~4.}

\begin{comment}{Comment R2.20}
Test forecast degradation and quantify scheduling performance under increasing
forecast error.
\end{comment}
\R{Controlled error is injected into the carbon signal at increasing magnitudes and
...
\begin{comment}{Comment R2.21}
Compare predicted-carbon, reactive and perfect-foresight scheduling using identical
optimization budgets.
\end{comment}
\R{All three are compared at an identical budget of 4{,}840 objective evaluations.
...
\begin{comment}{Comment R2.22}
Correct grammatical and wording errors.
\end{comment}
\R{The manuscript has been revised throughout. Specific instances addressed
...
explicit.}
\Where{Abbreviations block; throughout.}

\begin{comment}{Comment R2.24}
Define all abbreviations at first use.
\end{comment}
\R{An abbreviations block has been added after the keywords, defining nineteen
...
\Where{Abbreviations block, after the keywords.}

\begin{comment}{Comment R2.25}
Improve figure readability and include units, legends and error information.
\end{comment}
\R{All figures have been regenerated from the raw per-seed results at 300\,dpi.
...
\begin{comment}{Comment R2.26}
Add standard deviations or confidence intervals to relevant tables.
\end{comment}
\R{All result tables now report dispersion over 30 seeds. See Comment~R2.6.}
\Where{All results tables.}

\begin{comment}{Comment R2.27}
Provide complete forecasting-model hyperparameters.
\end{comment}
\R{Full hyperparameters are tabulated: look-back window, forecast horizons,
...
seed.}
\Where{Table~9, Section~5.10.}

\begin{comment}{Comment R2.28}
Correct the ``Availability of data and materials: NA'' statement because public
datasets are used.
\end{comment}
\R{Corrected. A full availability statement now names all three workload and five
carbon data sources, all of which are public, together with the archived artefact
DOI.}
\Where{Availability of Data and Materials.}

\begin{comment}{Comment R2.29}
Provide a persistent code repository link/DOI if permitted.
\end{comment}
\R{The artefact is archived with a persistent DOI:
...
\begin{comment}{Comment R2.30}
Clarify whether time-of-use tariff values are actually included in the reported
objective/results.
\end{comment}
\R{Clarified, and the honest answer is that the tariff is present but carries
...
\begin{comment}{Comment R2.31}
Use consistent units for energy, emissions, carbon intensity and monetary cost.
\end{comment}
\R{Units are now consistent throughout and stated in every table header: energy in
...
\Where{Throughout; all table headers.}

\begin{comment}{Comment R2.32}
Check reference metadata, DOI information and formatting.
\end{comment}
\R{Every entry in the bibliography was resolved against Crossref before use. The
...
\begin{comment}{Comment R3.1}
Add evaluation on multiple carbon intensity datasets and newer traces.
\end{comment}
\R{Five real regional carbon-intensity datasets are now used, and the NASA-iPSC
workload trace has been added alongside the Google trace. See Comment~R2.19.}
\Where{Section~3.1, Table~2; Section~5.12.}

\begin{comment}{Comment R3.2}
Include stronger baselines (at least one carbon-aware heuristic, one modern
scheduler).
\end{comment}
\R{Two published policies were implemented in the same simulator, using no fitness
...
insufficient.}
\Where{Section~5.3; Table~5.}

\begin{comment}{Comment R3.3}
Provide statistical significance tests for all comparisons.
\end{comment}
\R{Both Welch's $t$-test and the Mann--Whitney $U$ test are applied to every
...
\Where{Section~4.5 (Theorem~1, Corollaries 1--2, Proposition~1).}

\begin{comment}{Comment R3.9}
Include at least one additional carbon region (US or EU).
\end{comment}
\R{Four additional regions are included: the Republic of Ireland, Northern Ireland,
...
one.}
\Where{Section~3.1, Table~2; Section~5.12; Figure~4; Section~6 (Limitations).}

\begin{comment}{Comment R3.10}
Report results with standard deviations for all experiments.
\end{comment}
\R{All experiments report standard deviations and 95\,\% confidence intervals over
30 seeds, and per-seed values are released. See Comment~R2.6.}
\Where{All results tables; archived artefact.}

\begin{comment}{Comment R3.11}
...
\end{comment}
\R{The bibliography has been reviewed in full and expanded from 19 to 31 entries,
...
\Where{Section~2.4; Table~1; Bibliography.}
